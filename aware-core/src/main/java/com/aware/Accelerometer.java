...
public class Accelerometer extends Aware_Sensor implements SensorEventListener {
    ...
    public void onDestroy() {
        ...
        if (shouldDeleteDatabase) {
            getContentResolver().delete(Uri.parse("content://" + Accelerometer_Provider.AUTHORITY + "/" + DatabaseHelper.DROP_TABLE_URI), null, null);
        }
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (PERMISSIONS_OK) {
            if (mAccelerometer == null) {
                ...
                tryParseIntPreference(Aware_Preferences.FREQUENCY_ACCELEROMETER, 200000);
                tryParseDoublePreference(Aware_Preferences.THRESHOLD_ACCELEROMETER, 0.0);
